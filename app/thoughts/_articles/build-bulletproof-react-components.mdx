...
9. [Make It __Future__-Proof](#make-it-future-proof)
...
<BlockSideTitle title="Simple play/pause button">
...
But wrap it in [`<Activity>`](https://react.dev/reference/react/Activity), and the music keeps playing when hidden. `<Activity>` preserves DOM, and `<audio>` has DOM-level side effects React can’t automatically stop. Add cleanup with `useLayoutEffect`:

<BlockSideTitle title="useLayoutEffect cleanup pauses when hidden">
...
Now hidden components pause their media.

## Make It Future-Proof

A scatter plot that assigns random colors to points:

<BlockSideTitle title="useMemo caches the computed colors">

```jsx
function Scatter({ points }) {
  // [!code word:useMemo]
  const colored = useMemo(
    () => assignRandomColors(points),
    [points]
  )

  return <Canvas data={colored} />
}
```

</BlockSideTitle>

But `useMemo` is a performance hint, not a semantic guarantee. React discards cached values during HMR, and reserves the right to do so for offscreen components or features that don’t exist yet. If React discards the cache, colors change unexpectedly. Use state when correctness depends on persistence:
...
function Scatter({ points }) {
  ...
  // [!code highlight:3]
  useEffect(() => {
    setColored(assignRandomColors(points))
  }, [points])
